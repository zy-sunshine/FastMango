...
class MiLogger(object):
    ...
    @staticmethod
    def disable_print_log(Class):
        Class.__print_log = False
    ...
    def dolog(self, msg, *args, **kw):
        ...
        if CONF_DOLOG and self.__logf:
            ...
            self.__logf.flush()
        if self.__print_log and kw.has_key('pr'):
            kw['pr'](msg)
        self.__lock.release()
...
__logger_map = { 'ClientLogger': ('/var/log/mi/client.log', 'C:\\client.log'),
                              'ServerLogger_Short': ('/var/log/mi/server-short.log', 'C:\\server-short.log'),
                              'ServerLogger_Long': ('/var/log/mi/server-long.log', 'C:\\server-long.log'),
                              }
for __logger, __logpath in __logger_map.items():
    #### Create Logger classes
    if os.name == 'nt':
        exec('%s = GetSingleLoggerClass("%s", "%s")' % (__logger, __logger, __logpath[1]))
    elif os.name == 'posix':
        exec('%s = GetSingleLoggerClass("%s", "%s")' % (__logger, __logger, __logpath[0]))
